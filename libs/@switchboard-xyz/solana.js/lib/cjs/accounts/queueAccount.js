...
const SolanaClock_1 = require("../SolanaClock");
...
const bufferRelayAccount_1 = require("./bufferRelayAccount");
...
const queueDataBuffer_1 = require("./queueDataBuffer");
const vrfAccount_1 = require("./vrfAccount");
 ...
 */
class QueueAccount extends account_1.Account {
    constructor() {
        super(...arguments);
        /**
    ...
    static async load(program, publicKey) {
        const account = new QueueAccount(program, typeof publicKey === 'string' ? new web3_js_1.PublicKey(publicKey) : publicKey);
    ...
    async loadData() {
        ...
        this.dataBuffer = new queueDataBuffer_1.QueueDataBuffer(this.program, data.dataBuffer);
        return data;
    }
    /**
    ...
    static async createInstructions(program, payer, params) {
        ...
        const dataBuffer = params.dataBufferKeypair ?? web3_js_1.Keypair.generate();
        program.verifyNewKeypair(dataBuffer);
        const queueAccount = new QueueAccount(program, keypair.publicKey);
        queueAccount.dataBuffer = new queueDataBuffer_1.QueueDataBuffer(program, dataBuffer.publicKey);
        const queueSize = params.queueSize ?? 500;
        const queueDataSize = queueDataBuffer_1.QueueDataBuffer.getAccountSize(queueSize);
        ...
        const txn = new TransactionObject_1.TransactionObject(payer, [
            ...
            types.oracleQueueInit(program, {
                params: {
                    ...
                    varianceToleranceMultiplier: common_1.SwitchboardDecimal.fromBig(new common_1.Big(params.varianceToleranceMultiplier ?? 2)),
    ...
    payer, params) {
        ...
        const [oracleAccount, createOracleTxnObject] = await oracleAccount_1.OracleAccount.createInstructions(this.program, payer, {
            ...params,
            queueAccount: this,
        });
        const [permissionAccount, createPermissionTxnObject] = permissionAccount_1.PermissionAccount.createInstruction(this.program, payer, {
        ...
        if (params.enable &&
            ...
            const permissionSetTxn = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_1.PermitOracleHeartbeat(),
                enable: true,
        ...
        return [
            oracleAccount,
            TransactionObject_1.TransactionObject.pack([
    ...
    async createFeedInstructions(payer, params) {
        ...
        if (params.jobs && Array.isArray(params.jobs)) {
            for await (const job of params.jobs) {
                if ('data' in job) {
                    const [jobAccount, jobInit] = jobAccount_1.JobAccount.createInstructions(this.program, payer, {
                        data: job.data,
                        name: job.name ?? '',
                    ...
                    const jobAccount = new jobAccount_1.JobAccount(this.program, job.pubkey);
        ...
        const [aggregatorAccount, aggregatorInit] = await aggregatorAccount_1.AggregatorAccount.createInstruction(this.program, payer, {
            ...params,
            queueAccount: this,
        ...
        const [leaseAccount, leaseInit] = await leaseAccount_1.LeaseAccount.createInstructions(this.program, payer, {
        ...
        const [permissionAccount, permissionInit] = permissionAccount_1.PermissionAccount.createInstruction(this.program, payer, {
            granter: this.publicKey,
            authority: queueAuthorityPubkey,
        ...
        if (params.enable &&
            (params.queueAuthority || queueAuthorityPubkey.equals(payer))) {
            const permissionSetTxn = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_1.PermitOracleQueueUsage(),
                enable: true,
                queueAuthority: params.queueAuthority,
            });
        ...
        if (params.crankPubkey) {
            const [permissionAccount, permissionBump] = permissionAccount_1.PermissionAccount.fromSeed(this.program, queueAuthorityPubkey, this.publicKey, aggregatorAccount.publicKey);
            const leaseEscrow = this.program.mint.getAssociatedAddress(leaseAccount.publicKey);
            post.push(new TransactionObject_1.TransactionObject(payer, [
                types.crankPush(this.program, {
                    ...
                    dataBuffer: params.crankDataBuffer ??
                        (await new crankAccount_1.CrankAccount(this.program, params.crankPubkey).loadData()).dataBuffer,
                }),
            ], []));
        }
        if (params.historyLimit && params.historyLimit > 0) {
            const historyBufferInit = (await aggregatorHistoryBuffer_1.AggregatorHistoryBuffer.createInstructions(this.program, payer, {
                aggregatorAccount,
                maxSamples: params.historyLimit,
            }))[1];
            post.push(historyBufferInit);
        }
        const packed = TransactionObject_1.TransactionObject.pack([
            ...(pre.length ? TransactionObject_1.TransactionObject.pack(pre) : []),
            ...(txns.length ? TransactionObject_1.TransactionObject.pack(txns) : []),
            ...(post.length ? TransactionObject_1.TransactionObject.pack(post) : []),
        ]);
    ...
    async createCrankInstructions(payer, params) {
        return await crankAccount_1.CrankAccount.createInstructions(this.program, payer, {
            ...params,
            queueAccount: this,
        });
    }
    ...
    async createVrfInstructions(payer, params) {
        const queueAuthorityPubkey = params.queueAuthority
            ? params.queueAuthority.publicKey
            : params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const [vrfAccount, vrfInit] = await vrfAccount_1.VrfAccount.createInstructions(this.program, payer, {
        ...
        let [permissionAccount, permissionInit] = permissionAccount_1.PermissionAccount.createInstruction(this.program, payer, {
            granter: this.publicKey,
        ...
        if (params.enable &&
            ...
            const permissionSet = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_1.PermitVrfRequests(),
                enable: true,
                queueAuthority: params.queueAuthority,
            });
    ...
    async createBufferRelayerInstructions(payer, params) {
        ...
        else if (params.job instanceof web3_js_1.PublicKey) {
            const jobAccount = new jobAccount_1.JobAccount(this.program, params.job);
            // should we verify its a valid job account?
            job = jobAccount;
        }
        else if (params.job instanceof jobAccount_1.JobAccount) {
            job = params.job;
        }
        else {
            throw new Error(`Failed to create BufferRelayer job account. 'data' or 'pubkey' was not defined in jobDefinition`);
        }
        const [bufferAccount, bufferInit] = await bufferRelayAccount_1.BufferRelayerAccount.createInstructions(this.program, payer, {
            name: params.name,
    ...
    async createVrfLiteInstructions(payer, params) {
        const queueAuthorityPubkey = params.queueAuthority
            ...
            : params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const txns = [];
        const [vrfLite, vrfLiteInit] = await vrfLiteAccount_1.VrfLiteAccount.createInstruction(this.program, payer, {
            ...params,
            queueAccount: this,
        });
        txns.push(vrfLiteInit);
        const [permissionAccount] = permissionAccount_1.PermissionAccount.fromSeed(this.program, queueAuthorityPubkey, this.publicKey, vrfLite.publicKey);
        if (params.enable &&
            (params.queueAuthority || queueAuthorityPubkey.equals(payer))) {
            const permissionSet = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_1.PermitVrfRequests(),
    ...
    async loadOracles() {
        ...
        if (this.dataBuffer) {
            ...
            queue = new queueDataBuffer_1.QueueDataBuffer(this.program, queueData.dataBuffer);
        }
    ...
    async loadOracleAccounts(_oracles) {
        ...
        return await oracleAccount_1.OracleAccount.fetchMultiple(this.program, oraclePubkeys);
    }
    async loadActiveOracleAccounts(_queue) {
        const queue = _queue ?? (await this.loadData());
        const oracles = await this.loadOracleAccounts();
        const unixTimestamp = (await SolanaClock_1.SolanaClock.fetch(this.program.connection))
            .unixTimestamp;
        const timeout = unixTimestamp.sub(new common_1.BN(queue.oracleTimeout));
    ...
    setConfigInstruction(payer, params) {
        const multiplier = params.varianceToleranceMultiplier &&
            ...
            ? common_1.SwitchboardDecimal.fromBig(new common_1.Big(params.varianceToleranceMultiplier))
            : null;
...
exports.QueueAccount = QueueAccount;
 ...
 */
QueueAccount.getName = (queue) => (0, common_1.toUtf8)(queue.name);
/**
 * Returns the queue's metadata buffer in a stringified format.
 */
QueueAccount.getMetadata = (queue) => (0, common_1.toUtf8)(queue.metadata);
