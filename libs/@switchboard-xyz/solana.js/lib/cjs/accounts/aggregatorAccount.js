...
const aggregatorHistoryBuffer_1 = require("./aggregatorHistoryBuffer");
const crankAccount_1 = require("./crankAccount");
const jobAccount_1 = require("./jobAccount");
const leaseAccount_1 = require("./leaseAccount");
...
class AggregatorAccount extends account_1.Account {
    constructor() {
         ...
         */
        this.size = this.program.account.aggregatorAccountData.size;
    }
    decode(data) {
        try {
            ...
            return this.program.coder.decode(AggregatorAccount.accountName, data);
        }
    }
    /**
     ...
     */
    async loadData() {
        const data = await types.AggregatorAccountData.fetch(this.program, this.publicKey);
        if (data === null)
            throw new errors.AccountNotFoundError('Aggregator', this.publicKey);
        this.history = aggregatorHistoryBuffer_1.AggregatorHistoryBuffer.fromAggregator(this.program, data);
        return data;
    }
    get slidingWindowKey() {
        return web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('SlidingResultAccountData'), this.publicKey.toBytes()], this.program.programId)[0];
    }
    /** Load an existing AggregatorAccount with its current on-chain state */
    static async load(program, publicKey) {
        const account = new AggregatorAccount(program, typeof publicKey === 'string' ? new web3_js_1.PublicKey(publicKey) : publicKey);
    ...
    static async createInstruction(program, payer, params) {
        ...
        ixns.push(web3_js_1.SystemProgram.createAccount({
            ...
            lamports: await program.connection.getMinimumBalanceForRentExemption(program.account.aggregatorAccountData.size),
            programId: program.programId,
        }));
        ixns.push(types.aggregatorInit(program, {
            params: {
                ...
                varianceThreshold: types.SwitchboardDecimal.fromBig(new common_1.Big(params.varianceThreshold ?? 0)).borsh,
        ...
        const aggregatorInit = new TransactionObject_1.TransactionObject(payer, ixns, signers);
    ...
    static async create(program, params) {
        const [account, transaction] = await AggregatorAccount.createInstruction(program, program.walletPubkey, params);
     ...
     */
    async transferQueuePart1Instructions(payer, params) {
        const txns = [];
        ...
        const [newPermissionAccount] = this.getPermissionAccount(params.newQueue.publicKey, newQueue.authority);
        const newPermissionAccountInfo = await this.program.connection.getAccountInfo(newPermissionAccount.publicKey);
        if (newPermissionAccountInfo === null) {
            const [_newPermissionAccount, permissionInitTxn] = permissionAccount_1.PermissionAccount.createInstruction(this.program, payer, {
                authority: newQueue.authority,
                granter: params.newQueue.publicKey,
                grantee: this.publicKey,
            });
            (0, assert_1.default)(newPermissionAccount.publicKey.equals(_newPermissionAccount.publicKey));
            txns.push(permissionInitTxn);
        }
        const [newLeaseAccount] = this.getLeaseAccount(params.newQueue.publicKey);
        const newLeaseAccountInfo = await this.program.connection.getAccountInfo(newLeaseAccount.publicKey);
        if (newLeaseAccountInfo === null) {
            const [userTokenWallet, userWrap] = params.fundAmount && params.fundAmount > 0 && !params.funderTokenWallet
            ...
            const [_newLeaseAccount, leaseInit] = await leaseAccount_1.LeaseAccount.createInstructions(this.program, payer, {
                aggregatorAccount: this,
                queueAccount: params.newQueue,
                funderTokenWallet: params.funderTokenWallet ?? userTokenWallet ?? undefined,
                funderAuthority: params.funderAuthority ?? undefined,
                jobAuthorities,
                fundAmount: params.fundAmount ?? 0,
                withdrawAuthority: aggregator.authority,
                jobPubkeys: aggregator.jobPubkeysData.slice(0, aggregator.jobPubkeysSize),
            });
            (0, assert_1.default)(newLeaseAccount.publicKey.equals(_newLeaseAccount.publicKey));
            txns.push(leaseInit);
        }
        const packed = TransactionObject_1.TransactionObject.pack(txns);
        if (packed.length !== 1) {
            throw new Error(`QueueTransfer-Part1: Expected a single TransactionObject`);
        }
    ...
    async transferQueuePart1(params) {
        const [txn, permissionAccount, leaseAccount] = await this.transferQueuePart1Instructions(this.program.walletPubkey, params);
     ...
     */
    async transferQueuePart2Instructions(payer, params) {
        const newQueue = await params.newQueue.loadData();
        const [permissionAccount] = this.getPermissionAccount(params.newQueue.publicKey, newQueue.authority);
        if (!params.enable) {
            return [undefined, permissionAccount];
        }
        if (params.queueAuthority &&
            !params.queueAuthority.publicKey.equals(newQueue.authority)) {
            throw new errors.IncorrectAuthority(newQueue.authority, params.queueAuthority.publicKey);
        }
        if (!params.force) {
    ...
    async transferQueuePart2(params) {
        const [txn, permissionAccount] = await this.transferQueuePart2Instructions(this.program.walletPubkey, params);
        if (!txn) {
     ...
     */
    async transferQueuePart3Instructions(payer, params) {
        const txns = [];
        const newQueue = await params.newQueue.loadData();
        const aggregator = await this.loadData();
        ...
        if (!params.force) {
            await permissionAccount
                ...
                .then(permission => {
                if (!newQueue.unpermissionedFeedsEnabled &&
                    permission.permissions !== 2) {
                    throw new Error(`PermissionAccount missing required permissions to enable this queue`);
                }
            });
        }
        ...
        // set the feeds queue
        const setQueueTxn = new TransactionObject_1.TransactionObject(payer, [
            types.aggregatorSetQueue(this.program, { params: {} }, {
        ...
        if (oldLease.withdrawAuthority.equals(payer) && oldLeaseBalance > 0) {
            const withdrawTxn = await oldLeaseAccount.withdrawInstruction(payer, {
                ...
                withdrawWallet: this.program.mint.getAssociatedAddress(newLeaseAccount.publicKey),
            });
            txns.push(withdrawTxn);
        }
        return TransactionObject_1.TransactionObject.pack(txns);
    }
    /**
     * Transfer the feed to the new {@linkcode QueueAccount} and optionally push it on a crank. Must be signed by the Aggregator authority to approve the transfer.
     *
    ...
    async transferQueuePart3(params) {
        const txns = await this.transferQueuePart3Instructions(this.program.walletPubkey, params);
        const signatures = await this.program.signAndSendAll(txns, {
            skipPreflight: true,
        });
        return signatures;
    }
    async transferQueueInstructions(payer, params, opts) {
        const txns = [];
        const [part1, permissionAccount, leaseAccount] = await this.transferQueuePart1Instructions(payer, {
        ...
        txns.push(...part3);
        return [
            TransactionObject_1.TransactionObject.pack(txns, opts),
            permissionAccount,
            leaseAccount,
        ];
    }
    async transferQueue(params) {
        const [txns, permissionAccount, leaseAccount] = await this.transferQueueInstructions(this.program.walletPubkey, params);
        const signatures = await this.program.signAndSendAll(txns, {
            skipPreflight: true,
        });
    ...
    getLeaseAccount(queuePubkey) {
        const [leaseAccount, leaseBump] = leaseAccount_1.LeaseAccount.fromSeed(this.program, queuePubkey, this.publicKey);
    ...
    getAccounts(queueAccount, queueAuthority) {
        ...
        const [leaseAccount, leaseEscrow, leaseBump] = this.getLeaseAccount(queueAccount.publicKey);
        return {
            permissionAccount,
            permissionBump,
            leaseAccount,
     ...
     * ```
     */
    static decodeLatestTimestamp(aggregator) {
        if ((aggregator.latestConfirmedRound?.numSuccess ?? 0) === 0) {
     ...
     * ```
     */
    static decodeConfirmedRoundResults(aggregator) {
        if ((aggregator.latestConfirmedRound?.numSuccess ?? 0) === 0) {
            throw new Error('Aggregator currently holds no value.');
        }
        const results = [];
     ...
     * ```
     */
    getConfirmedRoundResults(aggregator) {
        return AggregatorAccount.decodeConfirmedRoundResults(aggregator).map(o => {
            return {
                oracleAccount: new oracleAccount_1.OracleAccount(this.program, o.oraclePubkeys),
                value: o.value,
            };
        });
    }
    ...
    produceJobsHash(jobs) {
        ...
        for (const job of jobs) {
            const jobHasher = crypto_1.default.createHash('sha256');
            jobHasher.update(common_1.OracleJob.encodeDelimited(job).finish());
            hash.update(jobHasher.digest());
        }
        return hash;
    }
    static decodeCurrentRoundOracles(aggregator) {
        return aggregator.currentRound.oraclePubkeysData.slice(0, aggregator.oracleRequestBatchSize);
    }
    async loadCurrentRoundOracles(aggregator) {
        return await Promise.all(AggregatorAccount.decodeCurrentRoundOracles(aggregator).map(async (o) => {
            const oracleAccount = new oracleAccount_1.OracleAccount(this.program, o);
            return {
    ...
    static decodeJobPubkeys(aggregator) {
        return aggregator.jobPubkeysData.slice(0, aggregator.jobPubkeysSize);
    }
    async loadJobs(aggregator) {
        const jobAccountDatas = await anchor.utils.rpc.getMultipleAccounts(this.program.connection, AggregatorAccount.decodeJobPubkeys(aggregator));
        return await Promise.all(jobAccountDatas.map(async (j) => {
            if (!j?.account) {
                throw new Error(`Failed to fetch account data for job ${j?.publicKey}`);
            }
            if (!j.account.owner.equals(this.program.programId)) {
                throw new errors.IncorrectOwner(this.program.programId, j.account.owner);
            }
            const jobAccount = new jobAccount_1.JobAccount(this.program, j.publicKey);
            const jobState = this.program.coder.decode('JobAccountData', j.account.data);
            return {
                account: jobAccount,
                state: jobState,
                job: common_1.OracleJob.decodeDelimited(jobState.data),
                weight: Math.max(aggregator.jobWeights[aggregator.jobPubkeysData.findIndex((x) => x.equals(j.publicKey))] ?? 1, 1),
            };
        }));
    }
    getJobHashes(jobs) {
     ...
     * ```
     */
    verifyConfig(aggregator, queue, target) {
        ...
        const endState = {
            ...
            minUpdateDelaySeconds: target.minUpdateDelaySeconds ?? aggregator.minUpdateDelaySeconds,
        };
        if (endState.minUpdateDelaySeconds < 5) {
            throw new errors.AggregatorConfigError('minUpdateDelaySeconds', 'must be greater than 5 seconds');
        }
        if (endState.minJobResults > aggregator.jobPubkeysSize) {
            throw new errors.AggregatorConfigError('minJobResults', `must be less than the number of jobs (${aggregator.jobPubkeysSize})`);
        }
        if (endState.batchSize > numberOfOracles) {
            throw new errors.AggregatorConfigError('oracleRequestBatchSize', `must be less than the number of oracles actively heartbeating on the queue (${numberOfOracles})`);
        }
        if (endState.minOracleResults > endState.batchSize) {
            throw new errors.AggregatorConfigError('minOracleResults', `must be less than the oracleRequestBatchSize (${endState.batchSize})`);
        }
    }
    /**
    ...
    async setConfigInstruction(payer, params, options) {
        if (!(params.force ?? false)) {
            const aggregator = await this.loadData();
            const queueAccount = new queueAccount_1.QueueAccount(this.program, aggregator.queuePubkey);
        ...
        const setConfigIxn = types.aggregatorSetConfig(this.program, {
            params: {
                name: params.name
                    ? [
                ...
                varianceThreshold: varianceThreshold >= 0
                    ? new types.BorshDecimal(types.SwitchboardDecimal.fromBig(new common_1.Big(varianceThreshold)))
                    : null,
        ...
        return new TransactionObject_1.TransactionObject(payer, [setConfigIxn], params.authority ? [params.authority] : [], options);
    }
    /**
    ...
    async setConfig(params, options) {
        const setConfigTxn = await this.setConfigInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(setConfigTxn);
        return txnSignature;
    }
    setQueueInstruction(payer, params, options) {
        const setQueueIxn = types.aggregatorSetQueue(this.program, {
            params: {},
        }, {
            aggregator: this.publicKey,
            authority: params.authority ? params.authority.publicKey : payer,
        ...
        return new TransactionObject_1.TransactionObject(payer, [setQueueIxn], params.authority ? [params.authority] : [], options);
    }
    async setQueue(params, options) {
        const setQueueTxn = this.setQueueInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(setQueueTxn);
        return txnSignature;
    }
    addJobInstruction(payer, params, options) {
        const authority = params.authority ? params.authority.publicKey : payer;
        const addJobIxn = types.aggregatorAddJob(this.program, { params: { weight: params.weight ?? 1 } }, {
            aggregator: this.publicKey,
            authority: authority,
            job: params.job.publicKey,
        });
        return new TransactionObject_1.TransactionObject(payer, [addJobIxn], params.authority ? [params.authority] : [], options);
    }
    async addJob(params, options) {
        const txn = this.addJobInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(txn);
        return txnSignature;
    }
    lockInstruction(payer, params, options) {
        return new TransactionObject_1.TransactionObject(payer, [
            types.aggregatorLock(this.program, { params: {} }, {
                aggregator: this.publicKey,
                authority: params.authority ? params.authority.publicKey : payer,
            }),
        ], params.authority ? [params.authority] : [], options);
    }
    async lock(params, options) {
        const lockTxn = this.lockInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(lockTxn);
        return txnSignature;
    }
    setAuthorityInstruction(payer, params, options) {
        return new TransactionObject_1.TransactionObject(payer, [
            types.aggregatorSetAuthority(this.program, { params: {} }, {
                aggregator: this.publicKey,
                authority: params.authority ? params.authority.publicKey : payer,
    ...
    updateJobWeightInstruction(payer, params, options) {
    ...
    async updateJobWeight(params, options) {
        const transaction = this.updateJobWeightInstruction(this.program.walletPubkey, params, options);
        const signature = await this.program.signAndSend(transaction);
        return signature;
    }
    removeJobInstruction(payer, params, options) {
        const authority = params.authority ? params.authority.publicKey : payer;
        const removeJobIxn = types.aggregatorRemoveJob(this.program, { params: { jobIdx: params.jobIdx } }, {
            aggregator: this.publicKey,
            authority: authority,
            job: params.job.publicKey,
        });
        return new TransactionObject_1.TransactionObject(payer, [removeJobIxn], params.authority ? [params.authority] : [], options);
    }
    async removeJob(params, options) {
        const removeJobTxn = this.removeJobInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(removeJobTxn);
        return txnSignature;
    }
    async openRoundInstruction(payer, params, options) {
        const aggregatorData = await this.loadData();
        const queueAccount = new queueAccount_1.QueueAccount(this.program, aggregatorData.queuePubkey);
        const queue = await queueAccount.loadData();
        const { permissionAccount, permissionBump, leaseAccount, leaseBump, leaseEscrow, } = this.getAccounts(queueAccount, queue.authority);
        const ixns = [];
        const payoutWallet = params?.payoutWallet ?? this.program.mint.getAssociatedAddress(payer);
        const payoutWalletAccountInfo = await this.program.connection.getAccountInfo(payoutWallet);
        if (payoutWalletAccountInfo === null) {
            const [createTokenAccountTxn] = this.program.mint.createAssocatedUserInstruction(payer);
            ixns.push(...createTokenAccountTxn.ixns);
        }
        ixns.push(types.aggregatorOpenRound(this.program, {
            params: {
        ...
        return new TransactionObject_1.TransactionObject(payer, ixns, [], options);
    }
    async openRound(params, options) {
        const openRoundTxn = await this.openRoundInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(openRoundTxn);
        return txnSignature;
    }
    saveResultInstructionSync(payer, params, options) {
        const [oraclePermissionAccount, oraclePermissionBump] = params.oraclePermission;
        if (params.oracleIdx < 0 || params.oracleIdx > params.oracles.length - 1) {
            throw new Error('Failed to find oracle in current round');
        }
        const saveResultIxn = types.aggregatorSaveResult(this.program, {
            params: {
        ...
        params.oracles.forEach(oracle => remainingAccounts.push(oracle.account.publicKey));
        params.oracles.forEach(oracle => remainingAccounts.push(oracle.state.tokenAccount));
        remainingAccounts.push(this.slidingWindowKey);
        saveResultIxn.keys.push(...remainingAccounts.map((pubkey) => {
            return { isSigner: false, isWritable: true, pubkey };
        }));
        return new TransactionObject_1.TransactionObject(payer, [saveResultIxn], [], options);
    }
    async saveResultInstruction(payer, params, options) {
        const aggregator = params.aggregator ?? (await this.loadData());
        const oracles = params.oracles ?? (await this.loadCurrentRoundOracles(aggregator));
        const oracleIdx = params.oracleIdx ??
            oracles.findIndex(o => o.account.publicKey.equals(params.oracleAccount.publicKey));
        if (oracleIdx < 0 || oracleIdx > oracles.length - 1) {
            throw new Error('Failed to find oracle in current round');
        }
        const queueAccount = params.queueAccount ??
            new queueAccount_1.QueueAccount(this.program, aggregator.queuePubkey);
        const queueAuthority = params.queueAuthority ?? (await queueAccount.loadData()).authority;
        const [oraclePermissionAccount, oraclePermissionBump] = params.oraclePermission ??
            params.oracleAccount.getPermissionAccount(queueAccount.publicKey, queueAuthority);
        const accounts = params.permissionAccount === undefined ||
        ...
        const saveResultTxn = this.saveResultInstructionSync(payer, {
            ...accounts,
            queueAccount,
            queueAuthority,
            jobs: params.jobs,
            historyBuffer: aggregator.historyBuffer.equals(web3_js_1.PublicKey.default)
                ? undefined
    ...
    async saveResult(params, options) {
        const saveResultTxn = await this.saveResultInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(saveResultTxn);
        return txnSignature;
    }
    async fetchAccounts(_aggregator, _queueAccount, _queue, commitment = 'confirmed') {
        const aggregator = _aggregator ?? (await this.loadData());
        const queueAccount = _queueAccount ?? new queueAccount_1.QueueAccount(this.program, aggregator.queuePubkey);
        const queue = _queue ?? (await queueAccount.loadData());
        const { permissionAccount, permissionBump, leaseAccount, leaseEscrow, leaseBump, } = this.getAccounts(queueAccount, queue.authority);
        const jobPubkeys = aggregator.jobPubkeysData.slice(0, aggregator.jobPubkeysSize);
        const accountInfos = await anchor.utils.rpc.getMultipleAccounts(this.program.connection, [
        ...
        if (!permissionAccountInfo || !permissionAccountInfo.account) {
            throw new Error(`PermissionAccount has not been created yet for this aggregator`);
        }
        const permission = types.PermissionAccountData.decode(permissionAccountInfo.account.data);
        const leaseAccountInfo = accountInfos.shift();
        if (!leaseAccountInfo || !leaseAccountInfo.account) {
            throw new Error(`LeaseAccount has not been created yet for this aggregator`);
        }
        ...
        if (!leaseEscrowAccountInfo || !leaseEscrowAccountInfo.account) {
            throw new Error(`LeaseAccount escrow has not been created yet for this aggregator`);
        }
        const leaseEscrowAccount = spl.unpackAccount(leaseEscrow, leaseEscrowAccountInfo.account);
        const jobs = [];
    ...
    async toAccountsJSON(_aggregator, _queueAccount, _queue) {
        const accounts = await this.fetchAccounts(_aggregator, _queueAccount, _queue);
        return {
            publicKey: this.publicKey,
    ...
    setSlidingWindowInstruction(payer, params, options) {
        return new TransactionObject_1.TransactionObject(payer, [
            types.aggregatorSetResolutionMode(this.program, {
    ...
    async setSlidingWindow(params, options) {
        const setSlidingWindowTxn = this.setSlidingWindowInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(setSlidingWindowTxn);
        return txnSignature;
    }
    async openRoundAndAwaitResult(params, timeout = 30000, options) {
        const aggregator = params?.aggregator ?? (await this.loadData());
        ...
        const statePromise = (0, common_1.promiseWithTimeout)(timeout, new Promise((resolve) => {
            ws = this.onChange(aggregator => {
                ...
                if (aggregator.latestConfirmedRound.roundOpenSlot.gt(currentRoundOpenSlot) &&
        ...
        const openRoundSignature = await this.openRound(params, options).catch(async (error) => {
            await closeWebsocket();
     ...
     */
    async nextRound(roundOpenSlot, timeout = 30000) {
        const slot = roundOpenSlot ?? (await this.loadData()).currentRound.roundOpenSlot;
        let ws;
        let result;
        try {
            result = await (0, common_1.promiseWithTimeout)(timeout, new Promise((resolve) => {
                ws = this.onChange(aggregator => {
     ...
     */
    async loadHistory(startTimestamp, endTimestamp) {
        if (!this.history) {
            this.history = new aggregatorHistoryBuffer_1.AggregatorHistoryBuffer(this.program, (await this.loadData()).historyBuffer);
        }
    ...
    static async fetchMultiple(program, publicKeys, commitment = 'confirmed') {
        ...
        for (const accountInfo of accountInfos) {
            ...
            try {
                ...
                const data = types.AggregatorAccountData.decode(accountInfo.account.data);
     ...
     */
    static calculatePriorityFee(aggregator, timestamp = Math.round(Date.now() / 1000), baseFee = 0 // base compute unit price
    ) {
        // parse defaults
        const lastUpdateTimestamp = aggregator.latestConfirmedRound.roundOpenTimestamp.gt(new common_1.BN(0))
        ...
        const maxPriorityFeeMultiplier = Math.max(1, aggregator.maxPriorityFeeMultiplier);
        // calculate staleness multiplier
        const multiplier = Math.min((timestamp - lastUpdateTimestamp) / priorityFeeBumpPeriod, maxPriorityFeeMultiplier);
    ...
    async fetchBalance(leaseEscrow, queuePubkey) {
        const escrowPubkey = leaseEscrow ??
            this.getLeaseAccount(queuePubkey ?? (await this.loadData()).queuePubkey)[1];
    ...
    async fundUpToInstruction(payer, fundUpTo, disableWrap = false) {
        const [leaseAccount, leaseEscrow] = this.getLeaseAccount((await this.loadData()).queuePubkey);
    ...
    static async fundMultipleUpToInstructions(payer, fundUpTo, aggregators, options) {
        ...
        for (const aggregator of aggregators) {
            const [depositTxn, depositAmount] = await aggregator.fundUpToInstruction(payer, fundUpTo, true);
            if (depositTxn && depositAmount) {
                txns.push(depositTxn);
                wrapAmount = wrapAmount + depositAmount;
            }
        }
        const [_, wrapTxn] = await program.mint.getOrCreateWrappedUserInstructions(payer, {
            fundUpTo: wrapAmount,
        });
        if (wrapTxn) {
            txns.unshift(wrapTxn);
        }
        return TransactionObject_1.TransactionObject.pack(txns, options);
    }
    /** Fund multiple aggregator account lease's up to a given balance */
    static async fundMultipleUpTo(fundUpTo, aggregators, options) {
        if (aggregators.length === 0) {
            throw new Error(`No aggregator accounts provided`);
        }
        const program = aggregators[0].program;
        const txns = await AggregatorAccount.fundMultipleUpToInstructions(program.walletPubkey, fundUpTo, aggregators, options);
        const txnSignatures = await program.signAndSendAll(txns);
        return txnSignatures;
    }
    async closeInstructions(payer, params, opts) {
        if (this.program.cluster === 'mainnet-beta') {
            throw new Error(`Aggregators can only be closed with the devnet version of Switchboard`);
        }
        ...
        const aggregator = await this.loadData();
        const [queueAccount, queue] = await queueAccount_1.QueueAccount.load(this.program, aggregator.queuePubkey);
        const slidingWindowKey = this.slidingWindowKey;
        const slidingWindowAccountInfo = await this.program.connection.getAccountInfo(slidingWindowKey);
        const { permissionAccount, permissionBump, leaseAccount, leaseBump, leaseEscrow, } = this.getAccounts(queueAccount, queue.authority);
        const crankPubkey = aggregator.crankPubkey.equals(web3_js_1.PublicKey.default)
            ? null
            : aggregator.crankPubkey;
        let dataBuffer = null;
        if (crankPubkey !== null) {
            const [crankAccount, crank] = await crankAccount_1.CrankAccount.load(this.program, crankPubkey);
        ...
        const closeIxn = await this.program._program.methods
            .aggregatorClose({
            stateBump: this.program.programState.bump,
            permissionBump: permissionBump,
            leaseBump: leaseBump,
        })
            .accounts(accounts // compiler expects all types to be pubkeys
        )
            .instruction();
        const closeTxn = tokenWalletInit
            ? tokenWalletInit.add(closeIxn, params?.authority ? [params.authority] : undefined)
            : new TransactionObject_1.TransactionObject(payer, [closeIxn], params?.authority ? [params.authority] : []);
        // add txn options
        return new TransactionObject_1.TransactionObject(closeTxn.payer, closeTxn.ixns, closeTxn.signers, opts);
    }
    async close(params, opts) {
        const closeTxn = await this.closeInstructions(this.program.walletPubkey, params, opts);
        const txnSignature = await this.program.signAndSend(closeTxn);
        return txnSignature;
    }
}
exports.AggregatorAccount = AggregatorAccount;
AggregatorAccount.accountName = 'AggregatorAccountData';
/**
 * Returns the aggregator's name buffer in a stringified format.
 */
AggregatorAccount.getName = (aggregator) => (0, common_1.toUtf8)(aggregator.name);
/**
 * Returns the aggregator's metadata buffer in a stringified format.
 */
AggregatorAccount.getMetadata = (aggregator) => (0, common_1.toUtf8)(aggregator.metadata);
AggregatorAccount.size = 3851;
//# sourceMappingURL=aggregatorAccount.js.map