import { loadKeypair, QueueAccount, SwitchboardNetwork, SwitchboardProgram, } from '.';
...
export function findAnchorTomlWallet(workingDir = process.cwd()) {
    let numDirs = 3;
    while (numDirs) {
...
export const DEFAULT_LOCALNET_NETWORK = {
    name: 'Localnet Queue',
    metadata: 'Localnet Metadata',
    keypair: Keypair.fromSecretKey(new Uint8Array([
    ...
    dataBufferKeypair: Keypair.fromSecretKey(new Uint8Array([
    ...
    oracle: {
        ...
        stakeAmount: 0,
        enable: true,
        stakingWalletKeypair: Keypair.fromSecretKey(new Uint8Array([
...
export class SwitchboardTestContext {
    ...
    static async load(connection, networkInitParams, walletPath, programId) {
        ...
        const wallet = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(walletFsPath, 'utf-8'))));
        const walletBalance = await connection.getBalance(wallet.publicKey);
        if (walletBalance === 0) {
            throw new Error(`Wallet is empty, balance: ${walletBalance}, wallet: ${wallet.publicKey.toBase58()}`);
        }
        const program = await SwitchboardProgram.fromConnection(connection, wallet, programId);
        ...
        const networkParams = _.merge({ reward: 0, minStake: 0, size: 10 }, networkInit, {
            authority: undefined,
            oracles: [
                networkInit.oracle
        ...
        try {
            if ('keypair' in networkParams) {
                ...
                const [queueAccount, queue] = await QueueAccount.load(program, queuePubkey);
