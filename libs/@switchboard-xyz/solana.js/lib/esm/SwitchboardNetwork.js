import { AggregatorAccount, BufferRelayerAccount, CrankAccount, JobAccount, LeaseAccount, OracleAccount, PermissionAccount, ProgramStateAccount, QueueAccount, VrfAccount, } from './accounts';
import { AggregatorAccountData, BufferRelayerAccountData, CrankAccountData, JobAccountData, LeaseAccountData, OracleAccountData, OracleQueueAccountData, PermissionAccountData, SbState, VrfAccountData, } from './generated';
 ...
 *
 */
export class SwitchboardNetwork {
    constructor(
    ...
    static find(program, networkName = 'default', switchboardDir = path.join(process.cwd(), '.switchboard')) {
        ...
        const obj = JSON.parse(fs.readFileSync(networkFile, 'utf-8'));
   ...
   */
    static async createInstructions(program, payer, params) {
        ...
        const [programState, stateBump, programInit] = await ProgramStateAccount.getOrCreateInstructions(program, payer);
        if (programInit) {
            txns.push(programInit);
        }
        // create a new queue
        const [queueAccount, queueInit] = await QueueAccount.createInstructions(program, payer, { ...params, authority: queueAuthorityPubkey });
        txns.push(queueInit);
        // create the cranks
        ...
        const oracles = await Promise.all((params.oracles ?? []).map(async (oracleInitParams) => {
            ...
            const [oraclePermissionAccount, oraclePermissionBump] = PermissionAccount.fromSeed(program, queueAuthorityPubkey, queueAccount.publicKey, oracleAccount.publicKey);
            return [
                oracleInit,
                {
        ...
        const aggregators = await Promise.all((params.aggregators ?? []).map(async (feedInitParams) => {
            ...
            const [aggregatorPermissionAccount, aggregatorPermissionBump] = PermissionAccount.fromSeed(program, queueAuthorityPubkey, queueAccount.publicKey, aggregatorAccount.publicKey);
            const [aggregatorLeaseAccount, aggregatorLeaseBump] = LeaseAccount.fromSeed(program, queueAccount.publicKey, aggregatorAccount.publicKey);
            return [
                aggregatorInit,
                {
        ...
        const vrfs = await Promise.all((params.vrfs ?? []).map(async (vrfInitParams) => {
            ...
            const [vrfPermissionAccount, vrfPermissionBump] = PermissionAccount.fromSeed(program, queueAuthorityPubkey, queueAccount.publicKey, vrfAccount.publicKey);
            return [
                vrfInit,
                {
                    account: vrfAccount,
        ...
        const bufferRelayers = await Promise.all((params.bufferRelayers ?? []).map(async (bufferRelayerInitParams) => {
            ...
            const [bufferRelayerPermissionAccount, bufferRelayerPermissionBump] = PermissionAccount.fromSeed(program, queueAuthorityPubkey, queueAccount.publicKey, bufferRelayerAccount.publicKey);
            return [
                bufferRelayerInit,
                {
   ...
   */
    ...
    async load() {
        ...
        const programState = {
            ...this.programState,
            state: SbState.decode(accountInfos.get(this.programState.account.publicKey.toBase58())
                ?.data ?? Buffer.from('')),
        };
        const queue = {
            ...this.queue,
            state: OracleQueueAccountData.decode(accountInfos.get(this.queue.account.publicKey.toBase58())?.data ??
                Buffer.from('')),
        };
        const cranks = this.cranks.map(crank => {
            return {
                ...crank,
                state: CrankAccountData.decode(accountInfos.get(crank.account.publicKey.toBase58())?.data ??
                    Buffer.from('')),
            };
        });
        const oracles = this.oracles.map(oracle => {
            return {
                ...oracle,
                state: OracleAccountData.decode(accountInfos.get(oracle.account.publicKey.toBase58())?.data ??
                    Buffer.from('')),
                permission: {
                    ...oracle.permission,
                    state: PermissionAccountData.decode(accountInfos.get(oracle.permission.account.publicKey.toBase58())
                        ?.data ?? Buffer.from('')),
                },
            };
        });
        const aggregators = this.aggregators.map(aggregator => {
            return {
                ...aggregator,
                state: AggregatorAccountData.decode(accountInfos.get(aggregator.account.publicKey.toBase58())?.data ??
                    Buffer.from('')),
                permission: {
                    ...aggregator.permission,
                    state: PermissionAccountData.decode(accountInfos.get(aggregator.permission.account.publicKey.toBase58())?.data ?? Buffer.from('')),
                },
                lease: {
                    ...aggregator.lease,
                    state: LeaseAccountData.decode(accountInfos.get(aggregator.lease.account.publicKey.toBase58())
                        ?.data ?? Buffer.from('')),
                },
            };
        });
        const vrfs = this.vrfs.map(vrf => {
            return {
                ...vrf,
                state: VrfAccountData.decode(accountInfos.get(vrf.account.publicKey.toBase58())?.data ??
                    Buffer.from('')),
                permission: {
                    ...vrf.permission,
                    state: PermissionAccountData.decode(accountInfos.get(vrf.permission.account.publicKey.toBase58())
                        ?.data ?? Buffer.from('')),
                },
            };
        });
        const bufferRelayers = this.bufferRelayers.map(bufferRelayer => {
            return {
                ...bufferRelayer,
                state: BufferRelayerAccountData.decode(accountInfos.get(bufferRelayer.account.publicKey.toBase58())
                    ?.data ?? Buffer.from('')),
                permission: {
                    ...bufferRelayer.permission,
                    state: PermissionAccountData.decode(accountInfos.get(bufferRelayer.permission.account.publicKey.toBase58())?.data ?? Buffer.from('')),
                },
            };
        });
        const jobPublicKeys = [];
    ...
    static from(program, obj) {
        ...
        if ('publicKey' in obj.queue) {
            ...
            const queueKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(obj.queue.keypair)));
        ...
        if ('authority' in obj.queue && typeof obj.queue.authority === 'string') {
            ...
            const queueAuthorityKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(obj.queue.queueAuthorityKeypair)));
        ...
        if ('cranks' in obj && Array.isArray(obj.cranks)) {
            for (const crank of obj.cranks ?? []) {
                if ('publicKey' in crank) {
                    ...
                    const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(crank.keypair)));
        ...
        if ('oracles' in obj && Array.isArray(obj.oracles)) {
            for (const oracle of obj.oracles ?? []) {
                ...
                if ('publicKey' in oracle) {
                    ...
                    const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(oracle.stakingWalletKeypair)));
                    [account] = OracleAccount.fromSeed(program, queueAccount.publicKey, keypair.publicKey);
                }
                if (account) {
                    const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queueAuthority, queueAccount.publicKey, account.publicKey);
                    oracles.push({
                        account,
                        permission: {
        ...
        if ('aggregators' in obj && Array.isArray(obj.aggregators)) {
            for (const aggregator of obj.aggregators ?? []) {
                ...
                if ('publicKey' in aggregator) {
                    ...
                    const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(aggregator.keypair)));
                    account = new AggregatorAccount(program, keypair.publicKey);
                }
                if (account) {
                    const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queueAuthority, queueAccount.publicKey, account.publicKey);
                    const [leaseAccount, leaseBump] = LeaseAccount.fromSeed(program, queueAccount.publicKey, account.publicKey);
                    aggregators.push({
                        account,
                        permission: {
                            account: permissionAccount,
        ...
        if ('vrfs' in obj && Array.isArray(obj.vrfs)) {
            for (const vrf of obj.vrfs ?? []) {
                ...
                if ('publicKey' in vrf) {
                    ...
                    const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(vrf.keypair)));
        ...
        if ('bufferRelayers' in obj && Array.isArray(obj.bufferRelayers)) {
            for (const bufferRelayer of obj.bufferRelayers ?? []) {
                ...
                if ('publicKey' in bufferRelayer) {
                    ...
                    const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(bufferRelayer.keypair)));
    ...
    static async fromQueue(queueAccount) {
        ...
        const [programStateAccount, programStateBump] = ProgramStateAccount.fromSeed(program);
        ...
        const oracles = Array.from(accounts.oracles)
            ...
            const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queue.authority, queueAccount.publicKey, oracleAccount.publicKey);
        ...
        const aggregators = Array.from(accounts.aggregators)
            ...
            const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queue.authority, queueAccount.publicKey, aggregatorAccount.publicKey);
            const permission = accounts.permissions.get(permissionAccount.publicKey.toBase58());
            if (!permission) {
                return undefined;
            }
            const [leaseAccount, leaseBump] = LeaseAccount.fromSeed(program, queueAccount.publicKey, aggregatorAccount.publicKey);
        ...
        const vrfs = Array.from(accounts.vrfs)
            ...
            const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queue.authority, queueAccount.publicKey, vrfAccount.publicKey);
            const permission = accounts.permissions.get(permissionAccount.publicKey.toBase58());
            if (!permission) {
                return undefined;
            }
        ...
        const bufferRelayers = Array.from(accounts.bufferRelayers)
            ...
            const bufferRelayerAccount = new BufferRelayerAccount(program, bufferRelayerKey);
            const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queue.authority, queueAccount.publicKey, bufferRelayerAccount.publicKey);
            const permission = accounts.permissions.get(permissionAccount.publicKey.toBase58());
            if (!permission) {
                return undefined;
            }
        ...
        const jobs = Array.from(accounts.jobs)
            .filter(([jobKey]) => jobPublicKeys.findIndex(j => j.equals(new PublicKey(jobKey))) !== -1)
            .map(([jobKey, job]) => {
...
export class LoadedSwitchboardNetwork {
    get program() {
        return this.queue.account.program;
    }
    constructor(fields) {
