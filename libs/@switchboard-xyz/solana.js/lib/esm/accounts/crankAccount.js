 ...
 */
export class CrankAccount extends Account {
    constructor() {
        super(...arguments);
        /**
    ...
    static async load(program, publicKey) {
        const account = new CrankAccount(program, typeof publicKey === 'string' ? new PublicKey(publicKey) : publicKey);
    ...
    static async createInstructions(program, payer, params) {
        ...
        const crankInit = new TransactionObject(payer, [
    ...
    pushInstructionSync(payer, params) {
        const queueAccount = new QueueAccount(this.program, params.crank.queuePubkey);
    ...
    async popInstruction(payer, params) {
        ...
        const txnObject = new TransactionObject(payer, [crankPopIxn], []);
        return txnObject;
    }
    async pop(params) {
    ...
    async loadCrank(sorted = true) {
        if (!this.dataBuffer) {
            this.dataBuffer = new CrankDataBuffer(this.program, (await this.loadData()).dataBuffer);
        }
    ...
    async fetchAccounts(_crank, _queueAccount, _queue) {
        ...
        const queueAccount = _queueAccount ?? new QueueAccount(this.program, crank.queuePubkey);
        const queue = _queue ?? (await queueAccount.loadData());
        const crankRows = await this.loadCrank();
        const aggregatorPubkeys = crankRows.map(r => r.pubkey);
        const aggregators = await AggregatorAccount.fetchMultiple(this.program, aggregatorPubkeys);
        return {
            crank: {
