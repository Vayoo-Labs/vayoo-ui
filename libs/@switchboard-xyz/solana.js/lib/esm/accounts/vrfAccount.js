 ...
 * Result: [u8;32]
 */
export class VrfAccount extends Account {
    constructor() {
        super(...arguments);
        /**
    ...
    static async load(program, publicKey) {
        const account = new VrfAccount(program, typeof publicKey === 'string' ? new PublicKey(publicKey) : publicKey);
    ...
    async requestRandomnessInstruction(payer, params, options) {
        ...
        const queueAccount = params.queueAccount ?? new QueueAccount(this.program, vrf.oracleQueue);
        ...
        const requestRandomness = new TransactionObject(payer, [
    ...
    setCallbackInstruction(payer, params) {
        const authorityPubkey = params.authority instanceof PublicKey
            ? params.authority
    ...
    async fetchAccounts(_vrf, _queueAccount, _queue) {
        ...
        const queueAccount = _queueAccount ?? new QueueAccount(this.program, vrf.oracleQueue);
    ...
    async closeAccountInstruction(payer, params) {
        ...
        const vrfClose = new TransactionObject(payer, [
            vrfCloseAction(this.program, {
                params: {
                    stateBump: this.program.programState.bump,
