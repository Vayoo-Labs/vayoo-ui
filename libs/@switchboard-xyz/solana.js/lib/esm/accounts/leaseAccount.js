 ...
 */
export class LeaseAccount extends Account {
    constructor() {
        super(...arguments);
        /**
    ...
    static async load(program, queue, aggregator) {
        const [account, bump] = LeaseAccount.fromSeed(program, typeof queue === 'string' ? new PublicKey(queue) : queue, typeof aggregator === 'string' ? new PublicKey(aggregator) : aggregator);
    ...
    static fromSeed(program, queue, aggregator) {
        const [publicKey, bump] = PublicKey.findProgramAddressSync([Buffer.from('LeaseAccountData'), queue.toBytes(), aggregator.toBytes()], program.programId);
    ...
    static async createInstructions(program, payer, params) {
        ...
        txns.push(new TransactionObject(payer, escrowBalance === null // lease might already exist if account was closed and re-opened
    ...
    async extendInstruction(payer, params) {
        ...
        return new TransactionObject(payer, [leaseExtend], params.funderAuthority ? [params.funderAuthority] : []);
    }
    async extend(params) {
    ...
    async fetchAccounts(_lease) {
        ...
        const aggregatorAccount = new AggregatorAccount(this.program, lease.aggregator);
