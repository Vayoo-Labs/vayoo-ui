...
export class VrfPoolAccount extends Account {
    constructor() {
        super(...arguments);
    ...
    static async createInstruction(program, payer, params) {
        ...
        const vrfPoolInitTxn = new TransactionObject(payer, [
    ...
    async pushNewInstruction(payer, params) {
        ...
        const queueAccount = params?.queueAccount ?? new QueueAccount(this.program, vrfPool.queue);
    ...
    async pushInstruction(payer, params) {
        ...
        const [queueAccount, queue] = await QueueAccount.load(this.program, vrfPool.queue);
        ...
        return new TransactionObject(payer, [pushIxn], params.authority ? [params.authority] : []);
    }
    async push(params) {
    ...
    async popInstructions(payer, params) {
        ...
        return new TransactionObject(payer, [popIxn], params?.authority ? [params.authority] : []);
    }
    async pop(params) {
    ...
    getRemainingAccounts(vrfPool, queueAuthority, size = 7) {
        ...
        const remainingAccounts = _.flatten(accountMetas).sort((a, b) => Buffer.compare(a.pubkey.toBuffer(), b.pubkey.toBuffer()));
    ...
    async requestInstructions(payer, params, size = 7) {
        ...
        const requestTxn = new TransactionObject(payer, [requestIxn], params?.authority ? [params.authority] : []);
        return requestTxn;
    }
    ...
    async depositInstructions(payer, params) {
        ...
        const transferTxn = new TransactionObject(payer, [
            createTransferInstruction(userTokenAddress, this.getEscrow(), params.tokenAuthority?.publicKey ?? payer, this.program.mint.toTokenAmount(params.amount)),
    ...
    async requestAndAwaitEvent(params, timeout = 30000) {
        ...
        const eventPromise = promiseWithTimeout(timeout, new Promise((resolve) => {
