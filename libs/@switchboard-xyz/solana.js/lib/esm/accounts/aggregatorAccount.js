...
import { Big, BN, OracleJob, promiseWithTimeout, toUtf8, } from '@switchboard-xyz/common';
...
export class AggregatorAccount extends Account {
    ...
    get slidingWindowKey() {
        return PublicKey.findProgramAddressSync([Buffer.from('SlidingResultAccountData'), this.publicKey.toBytes()], this.program.programId)[0];
    }
    /** Load an existing AggregatorAccount with its current on-chain state */
    static async load(program, publicKey) {
        const account = new AggregatorAccount(program, typeof publicKey === 'string' ? new PublicKey(publicKey) : publicKey);
    ...
    static async createInstruction(program, payer, params) {
        ...
        ixns.push(types.aggregatorInit(program, {
            params: {
                ...
                varianceThreshold: types.SwitchboardDecimal.fromBig(new Big(params.varianceThreshold ?? 0)).borsh,
    ...
    async transferQueuePart1Instructions(payer, params) {
        ...
        if (newPermissionAccountInfo === null) {
            const [_newPermissionAccount, permissionInitTxn] = PermissionAccount.createInstruction(this.program, payer, {
                authority: newQueue.authority,
                granter: params.newQueue.publicKey,
                grantee: this.publicKey,
            });
            assert(newPermissionAccount.publicKey.equals(_newPermissionAccount.publicKey));
        ...
        if (newLeaseAccountInfo === null) {
            ...
            const [_newLeaseAccount, leaseInit] = await LeaseAccount.createInstructions(this.program, payer, {
    ...
    async transferQueuePart3Instructions(payer, params) {
        ...
        // set the feeds queue
        const setQueueTxn = new TransactionObject(payer, [
    ...
    getLeaseAccount(queuePubkey) {
        const [leaseAccount, leaseBump] = LeaseAccount.fromSeed(this.program, queuePubkey, this.publicKey);
    ...
    async setConfigInstruction(payer, params, options) {
        if (!(params.force ?? false)) {
            const aggregator = await this.loadData();
            const queueAccount = new QueueAccount(this.program, aggregator.queuePubkey);
        ...
        const setConfigIxn = types.aggregatorSetConfig(this.program, {
            params: {
                ...
                varianceThreshold: varianceThreshold >= 0
                    ? new types.BorshDecimal(types.SwitchboardDecimal.fromBig(new Big(varianceThreshold)))
                    : null,
    ...
    setQueueInstruction(payer, params, options) {
        ...
        return new TransactionObject(payer, [setQueueIxn], params.authority ? [params.authority] : [], options);
    }
    ...
    addJobInstruction(payer, params, options) {
        ...
        return new TransactionObject(payer, [addJobIxn], params.authority ? [params.authority] : [], options);
    }
    ...
    removeJobInstruction(payer, params, options) {
        ...
        return new TransactionObject(payer, [removeJobIxn], params.authority ? [params.authority] : [], options);
    }
    ...
    async openRoundInstruction(payer, params, options) {
        ...
        const queueAccount = new QueueAccount(this.program, aggregatorData.queuePubkey);
        const queue = await queueAccount.loadData();
    ...
    async fetchAccounts(_aggregator, _queueAccount, _queue, commitment = 'confirmed') {
        ...
        const queueAccount = _queueAccount ?? new QueueAccount(this.program, aggregator.queuePubkey);
    ...
    async openRoundAndAwaitResult(params, timeout = 30000, options) {
        ...
        const statePromise = promiseWithTimeout(timeout, new Promise((resolve) => {
    ...
    async nextRound(roundOpenSlot, timeout = 30000) {
        ...
        let result;
        try {
            result = await promiseWithTimeout(timeout, new Promise((resolve) => {
                ws = this.onChange(aggregator => {
    ...
    async loadHistory(startTimestamp, endTimestamp) {
        if (!this.history) {
            this.history = new AggregatorHistoryBuffer(this.program, (await this.loadData()).historyBuffer);
        }
    ...
    static calculatePriorityFee(aggregator, timestamp = Math.round(Date.now() / 1000), baseFee = 0 // base compute unit price
        ...
        // parse defaults
        const lastUpdateTimestamp = aggregator.latestConfirmedRound.roundOpenTimestamp.gt(new BN(0))
    ...
    async closeInstructions(payer, params, opts) {
        ...
        const aggregator = await this.loadData();
        const [queueAccount, queue] = await QueueAccount.load(this.program, aggregator.queuePubkey);
        ...
        const { permissionAccount, permissionBump, leaseAccount, leaseBump, leaseEscrow, } = this.getAccounts(queueAccount, queue.authority);
        const crankPubkey = aggregator.crankPubkey.equals(PublicKey.default)
            ? null
            : aggregator.crankPubkey;
        let dataBuffer = null;
        if (crankPubkey !== null) {
            const [crankAccount, crank] = await CrankAccount.load(this.program, crankPubkey);
        ...
        const closeTxn = tokenWalletInit
            ...
            : new TransactionObject(payer, [closeIxn], params?.authority ? [params.authority] : []);
        // add txn options
        return new TransactionObject(closeTxn.payer, closeTxn.ixns, closeTxn.signers, opts);
    }
 ...
 */
AggregatorAccount.getName = (aggregator) => toUtf8(aggregator.name);
/**
 * Returns the aggregator's metadata buffer in a stringified format.
 */
AggregatorAccount.getMetadata = (aggregator) => toUtf8(aggregator.metadata);
