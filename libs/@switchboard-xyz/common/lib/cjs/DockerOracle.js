...
class DockerOracle {
    constructor(nodeImage, config, switchboardDirectory = path_1.default.join(process.cwd(), ".switchboard"), silent = false) {
        var _a, _b;
        ...
        if (!fs_1.default.existsSync(this.secretPath)) {
        ...
        // log config
        if (!fs_1.default.existsSync(this.switchboardDirectory)) {
            fs_1.default.mkdirSync(this.switchboardDirectory, { recursive: true });
        }
        this.logFile = path_1.default.join(this.switchboardDirectory, `docker.${this.nodeImage}.${Math.floor(this.timestamp / 1000)}.log`);
        // build image name from a hash of provided args
        const shaHash = crypto_1.default.createHash("sha256");
    ...
    saveLogs() {
        ...
        if (filteredLogs.length > 0) {
            if (fs_1.default.existsSync(this.logFile)) {
                fs_1.default.appendFileSync(this.logFile, "\r\n" + filteredLogs.join("\r\n"));
                this.logs = [];
            }
            else {
                fs_1.default.writeFileSync(this.logFile, filteredLogs.join("\r\n"));
                this.logs = [];
            }
        }
    }
...
exports.DockerOracle = DockerOracle;
function normalizeFsPath(fsPath) {
    return fsPath.startsWith("/") ||
        ...
        ? fsPath
        : fsPath.startsWith("~")
            ? path_1.default.join(os_1.default.homedir(), fsPath.slice(1))
            : path_1.default.join(process.cwd(), fsPath);
}
